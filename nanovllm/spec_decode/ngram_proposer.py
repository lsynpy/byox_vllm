...
class NgramProposer:
    def __init__(self, config: Config):
        ...
        assert config.speculative_config.prompt_lookup_min is not None
        assert config.speculative_config.prompt_lookup_max is not None

        # Minimum length of the n-gram to match.
        self.min_n = config.speculative_config.prompt_lookup_min
        # Maximum length of the n-gram to match.
        self.max_n = config.speculative_config.prompt_lookup_max
        # Number of tokens follow the match. If there are less than k
        # tokens follow the match, we will return the maximum amount of
        # tokens until the end.
        self.k = config.speculative_config.num_speculative_tokens
    ...
    def batch_propose(
        ...
        if num_ngram_requests := len(valid_ngram_requests):
            ...
            logger.info(
                "token_ids_cpu: %s, valid_ngram_draft: %s",
                token_ids_cpu.shape,  # TODO: shape (256, 2048), what's 256, 2048?
                self.valid_ngram_draft.shape,  # shape(256,2)
            )
