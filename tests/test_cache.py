...
def test_prefill():
    ...
    ########################################################################################
    # 1st seq: cache hit in the common prefix when the original block is still in use.
    ...
    ########################################################################################
    # 2nd seq: cache hit in the common prefix when the original block is already free.
    ...
    ########################################################################################
    # 3rd seq: cache miss and eviction.
    seq3 = Sequence([99] * 16 * 10, block_size)
    manager.allocate(seq3)

    assert seq3.num_blocks == 10
...
def test_decode():
    ...
    seq0 = Sequence(common_token_ids + unique_token_ids, block_size)
    manager.allocate(seq0)
